...
g <- ggplot(d, aes(date, 1, label = event)) +
  ...
  ylim(0.9, 1.4) +
  annotate("line",
    x = seq(lubridate::ymd("2020-03-07"), lubridate::ymd("2020-03-22"), by = "1 day"),
    y = 1
  ) +
...
#   ) +
#   coord_cartesian(clip = "off") +
#   geom_text(y = 1.015, angle = 0, hjust = 0, aes(colour = type), lineheight = .82) +
#   scale_color_manual(values = c(RColorBrewer::brewer.pal(6, "Blues")[5], "grey10")) +
#   guides(colour = FALSE) +
#   geom_text(y = 0.985, aes(label = date), angle = 0, hjust = 1) +
#   theme(plot.margin = margin(t = -10, r = -10, b = -10, l = 10, unit = "pt"))
#
# g + coord_flip(clip = "off")
# ggsave("figs-ms/timeline.png", width = 3.6, height = 5.6)
