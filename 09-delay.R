...
v <- round(quantile(states_timing$prevalence_peak - states_timing$start_decline, probs = c(0.05, 0.5, 0.95)), 1)

write_tex(v[[1]], "prevDelayFifteenthLwr")
write_tex(v[[2]], "prevDelayFifteenthMed")
write_tex(v[[3]], "prevDelayFifteenthUpr")

v <- round(quantile(states_timing$prevalence_peak - 12, probs = c(0.05, 0.5, 0.95)), 1)
