...
# get daily means
g1 <- g1 %>% group_by(date, region) %>% mutate(MeanTransit=mean(transit_stations_percent_change_from_baseline, na.rm=TRUE)) %>% mutate(MeanWorkplace=mean(workplaces_percent_change_from_baseline, na.rm=TRUE)) %>% mutate(MeanRec=mean(retail_and_recreation_percent_change_from_baseline, na.rm=TRUE))
...
ggplot(g1, aes(date, MeanTransit, group=region, col=region)) + geom_smooth(method="gam", se=FALSE) + geom_line(alpha=0.5) +
        labs(y="% change from baseline", x="") +theme_minimal()+ 
        scale_color_brewer(palette = "Dark2")


# Or with SE
ggplot(g1, aes(date, transit_stations_percent_change_from_baseline, group=region, col=region)) + geom_smooth(method="gam") + geom_line(aes(date, MeanTransit), alpha=0.5) +
        labs(y="% change from baseline", x="") +theme_minimal()+ 
        scale_color_brewer(palette = "Dark2")
